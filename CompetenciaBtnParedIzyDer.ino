...
#define CNY_6 39
#define LED_ON 13
#define RANGE 350+300//400
#define RANGEB 400+300
#define SUMA 1200
...
void loop() {
 ...
 digitalWrite(LED_ON, 0);
    ...
    switch (estado) {
        ...
        case 1:
        //MANO IZQUIERDA
                 if (sensor6 < RANGEB && sensor4 < RANGEB && sensor5 < 800+SUMA) { 
                 //if (sensor4 < RANGE+500 && sensor4 < RANGE && sensor5 < 800) {
     // SerialBT.println("IZQUIERDA");

      izquierdaRueda();
    } else if (sensor5 >  800+SUMA && sensor6 > RANGEB|| sensor4>800+SUMA && sensor3>800+SUMA) {
     // SerialBT.println("DERECHA");
      derecha();
    }else if(sensor2 > RANGEB && sensor4>RANGEB && sensor1 >RANGEB && sensor5>RANGEB){
    ...
    //  Serial.println("if 11"); SerialBT.println("AVANZAR");
     avanzarCI(sensor4);
        ...
        case 3:
        //MANO DERECHA
            if (sensor0 < RANGE && sensor2 < RANGE && sensor1 < 800+SUMA) {
            //  if (sensor2 < RANGE+400 && sensor2 < RANGE && sensor1 < 800) {
    //      SerialBT.println("DERECHA");
    
          derechaRueda();
        } else if (sensor1 > 800+SUMA && sensor0 > RANGE || sensor2 && sensor3>800+SUMA) {
     //     SerialBT.println("IZQUIERDA");
          izquierda();
...
void izquierdaRueda() {
  ...
  analogWrite(MOTOR_RIGTH_NEGATIVE, SPEED + SPEED * 0.7);
...
void derechaRueda() {
  ...
  /* Motor izequierdo con movimiento a enfrente*/
  analogWrite(MOTOR_LEFT_POSITIVE, 0);
  analogWrite(MOTOR_LEFT_NEGATIVE, SPEED + SPEED * 0.7);
...
void derecha() {
  ...
  analogWrite(MOTOR_RIGTH_POSITIVE, 45);//analogWrite(MOTOR_RIGTH_POSITIVE, 40);
  analogWrite(MOTOR_RIGTH_NEGATIVE, 0);

  analogWrite(MOTOR_LEFT_POSITIVE, 0);
  analogWrite(MOTOR_LEFT_NEGATIVE, 45);
}

void izquierda() {

  analogWrite(MOTOR_RIGTH_POSITIVE, 0);
 analogWrite(MOTOR_RIGTH_NEGATIVE, 50);
  //analogWrite(MOTOR_LEFT_POSITIVE, 40);
  analogWrite(MOTOR_LEFT_POSITIVE, 50);
  analogWrite(MOTOR_LEFT_NEGATIVE, 0);
}
